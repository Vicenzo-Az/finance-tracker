...
def get_monthly_detail(
    ...
    try:
        ...
        raise HTTPException(
            status_code=422, detail="Formato de mês inválido. Use YYYY-MM.")
    ...
    prev_income = round(float(prev_frame[prev_frame["type"] == "income"]["amount"].sum(
    )), 2) if not prev_frame.empty else 0.0
    prev_expense = round(float(prev_frame[prev_frame["type"] == "expense"]["amount"].sum(
    )), 2) if not prev_frame.empty else 0.0
    ...
    if not rec_df.empty:
        ...
        recurring_total = round(
            float(rec_frame["amount"].sum()), 2) if not rec_frame.empty else 0.0
    else:
        recurring_total = 0.0
